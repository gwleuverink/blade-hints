...
class InjectCss
{
    ...
    public function __invoke(RequestHandled $handled)
    {
        ...
        if (str_contains($html, '<!--[GLIMPSE-CSS]-->')) {
            return;
        }

        // Bundle it up & wrap in script tag
        $css = $this->wrapInStyleTag(
            file_get_contents(__DIR__ . '/../resources/glimpse.css')
        );
    ...
    protected function wrapInStyleTag($contents): string
    {
        return <<< HTML
        <!--[GLIMPSE-CSS]-->
        <style>
